...
//			};
        ...
        nodeLabels.filter(label -> label._1.startsWith("h"))
	        .foreachPartition(partition -> {
	            ...
	            while (partition.hasNext()) {
		            if (toAdd.size() >= 25) {
		            	TableWriteItems writeItems = new TableWriteItems(WEIGHTS_TABLE).withItemsToPut(toAdd);
	                ...
	                Tuple2<String,Tuple2<String, Double>> item = partition.next();
	                Item weight = new Item()
	                		.withPrimaryKey("link", item._1)
	                		.withString("userId", item._2._1.substring(1))
	                		.withDouble("weight", item._2._2);
...
//    	final long startTime = System.currentTimeMillis();
		final Adsorption ad = new Adsorption();
